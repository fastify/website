...
export function BenchmarkSection() {
  const max_value = Math.max(...benchmark_data.map((framework) => parseInt(framework.requests) || 0))
  return benchmark_data.map((framework) => (
    <BenchmarkInfo name={framework.name} req={framework.requests} progress_value={framework.requests / max_value} />
  ))
}

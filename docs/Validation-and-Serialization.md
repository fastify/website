...
> ℹ If you need to use the lastest version of Ajv (v8) you should read how to do it in the [`schemaController`](./02-Reference/Server.md#schemaController) section. It is explained the easier way to avoid to implement a custom validator.
...
The Ajv default configuration in Fastify doesn't support coercing array parameters in querystring. However, Fastify allows [`customOptions`](./02-Reference/Server.md#ajv) in Ajv instance. The `coerceTypes: 'array'` will coerce one parameter to a single element in array. Example:
...
> Refer to [`ajv options`](./02-Reference/Server.md#ajv) to check plugins format
...
This baseline configuration can be modified by providing [`ajv.customOptions`](./02-Reference/Server.md#ajv) to your Fastify factory.
